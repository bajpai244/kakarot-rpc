...
const runMadara = async () => {
  const madaraPath = resolve(
    process.cwd(),
    "../lib/madara/target/release/madara"
  );
...
const deploy_kakarot = async () => {
  ...
  const deployScriptPath = resolve(
    process.cwd(),
...
const run_rpc = async () => {
  const madaraDeploymentFilePath = resolve(
    process.cwd(),
    "../lib/kakarot/deployments/madara/deployments.json"
  );
  const madaraDeployments = JSON.parse(
    readFileSync(madaraDeploymentFilePath).toString()
  );
  ...
  const rpc = spawn(rpcPath, {
    env: {
      ...
      PROXY_ACCOUNT_CLASS_HASH:
        "0x4b9eef81a3f0a582dfed69be93196cedbff063e0fa206b34b4c2f06ac505f0c",
      RUST_LOG: "",
    },
  });
